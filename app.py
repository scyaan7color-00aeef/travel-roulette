...
# ─────────────────────────────────────────────
#  spots.csv から追加スポットを読み込んで SPOTS に統合
# ─────────────────────────────────────────────
_PREF_TO_REGION = {
    "北海道": "北海道",
...
def _load_csv_spots():
    try:
        ...
        for _, row in df.iterrows():
            if row["spot_name"] not in existing_names:
                result.append({
                    "name": row["spot_name"],
                    "pref": row["prefecture"],
                    "region": _PREF_TO_REGION.get(row["prefecture"], "その他"),
...
# ─────────────────────────────────────────────
#  spots.csv 読み込み＋プラン生成
# ─────────────────────────────────────────────
@st.cache_data
def load_spots_csv():
    return pd.read_csv("spots.csv")

def generate_plan(prefecture, time_limit_min):
    """指定都道府県・時間内に収まるスポット組み合わせをランダム生成"""
    df = load_spots_csv()
    pref_spots = df[df["prefecture"] == prefecture].to_dict("records")
    if not pref_spots:
        return []
    # スポット数が最大になるサブセットを優先して探索
    valid_plans = []
    for r in range(len(pref_spots), 0, -1):
        for combo in combinations(pref_spots, r):
            total = sum(s["duration_min"] + s["transfer_min"] for s in combo)
            # 最後のスポットは移動不要なので最大 transfer_min を引く
            max_transfer = max(s["transfer_min"] for s in combo)
            if total - max_transfer <= time_limit_min:
                valid_plans.append(list(combo))
        if valid_plans:
            break  # 最大スポット数の組み合わせが見つかった時点で終了
    if not valid_plans:
        return []
    plan = random.choice(valid_plans)
    random.shuffle(plan)
    return plan
...
.plan-meta { color: #888; font-size: 0.82rem; margin-bottom: 0.35rem; }
.plan-desc { font-size: 0.9rem; line-height: 1.75; color: #444; }
.plan-arrow { text-align: center; color: #bbb; font-size: 1.2rem; margin: 0.1rem 0; }
</style>
...
if "plan_hours" not in st.session_state:
    st.session_state.plan_hours = None

# ─────────────────────────────────────────────
#  UI ヘッダー
# ─────────────────────────────────────────────
st.title("🗾 旅行プラン ルーレット")
st.caption(f"全国47都道府県対応・{len(SPOTS)}スポットからランダムに観光地を提案します")
st.divider()
...
with tab_roulette:
    ...
    if st.session_state.spot is not None:
        ...
        # ── 観光プランボタン ──
        st.divider()
        st.markdown("#### ⏱️ この都道府県の観光プランを作成")
        pref_key = spot["pref"].split()[0]
        col_a, col_b = st.columns(2)
        with col_a:
            if st.button("🕒 3時間プラン", use_container_width=True):
                st.session_state.plan = generate_plan(pref_key, 180)
                st.session_state.plan_hours = 3
        with col_b:
            if st.button("🕔 5時間プラン", use_container_width=True):
                st.session_state.plan = generate_plan(pref_key, 300)
                st.session_state.plan_hours = 5

        if st.session_state.plan is not None:
            if len(st.session_state.plan) == 0:
                st.warning(f"{pref_key} のスポットデータが見つからないか、時間内に収まる組み合わせがありません。")
            else:
                plan = st.session_state.plan
                hours = st.session_state.plan_hours
                total = sum(s["duration_min"] + s["transfer_min"] for s in plan)
                actual_min = total - max(s["transfer_min"] for s in plan)
                st.markdown(
                    f"**{pref_key} の {hours}時間プラン** "
                    f"（{len(plan)} スポット・合計約 {actual_min} 分）"
                )
                for i, s in enumerate(plan):
                    is_last = i == len(plan) - 1
                    meta = f"⏱️ 所要 {s['duration_min']}分"
                    if not is_last:
                        meta += f"　🚗 次のスポットまで約 {s['transfer_min']}分"
                    st.markdown(f"""
<div class="plan-card">
  <div class="plan-spot-name">📍 {s['spot_name']}</div>
  <div class="plan-meta">{meta}</div>
  <div class="plan-desc">{s['description']}</div>
</div>
""", unsafe_allow_html=True)
                    if not is_last:
                        st.markdown('<div class="plan-arrow">↓</div>', unsafe_allow_html=True)
